...
# from flask_cors import CORS
# from threading import Thread
# import keyboard  # Keyboard module to handle WASD input
...
# def A():
#     print(f"Function A triggered! Attention is high 👀 (more than {ranges['high']})")
#     asyncio.run(send_command_via_websocket('A'))

# def B():
#     print(f"Function B triggered! Attention is medium-high (more than {ranges['medium']} but less than or equal to {ranges['high']})")
#     asyncio.run(send_command_via_websocket('B'))

# def C():
#     print(f"Function C triggered! Attention is medium-low (more than {ranges['low']} but less than or equal to {ranges['medium']})")
#     asyncio.run(send_command_via_websocket('C'))

# def D():
#     print(f"Function D triggered! Attention is low 😴 (more than 0 but less than or equal to {ranges['low']})")
#     asyncio.run(send_command_via_websocket('D')) 

# # Function to map WASD keys to specific actions
# def handle_key_input():
#     while True:
#         try:
#             if keyboard.is_pressed('w'):
#                 A()
#                 time.sleep(0.5)  # Short delay to prevent rapid triggering
...
#     # Start handling keyboard inputs in the main thread
#     handle_key_input()
